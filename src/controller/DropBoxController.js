class DropBoxController{
    ...
    constructor(){
        ...
        this.currentFolder = ['hcode'];//variavel para ser usada para as pastas no firebase
        ...
        //criando um evento novo personalizado
    ...
    connectFirebase(){
        ...
        // Initialize Firebase
        var config = {
            apiKey: "AIzaSyBfjDr0uZt0DZtXPas83qXV8PcT--NU6bg",
            authDomain: "dropbox-clone-6221b.firebaseapp.com",
            databaseURL: "https://dropbox-clone-6221b.firebaseio.com",
            projectId: "dropbox-clone-6221b",
            storageBucket: "dropbox-clone-6221b.appspot.com",
            messagingSenderId: "114843190106"
        };
    ...
    //Metodo dos eventos dos botões
    initEvents(){

        //evento do botão "new folder"
        ...
        //evento do botão "Rename"
        this.btnRename.addEventListener('click', e=>{

            //como a função rename só pode renomear um item, pega o indice 0
        ...
        //lógica para mostrar os botões ou não conforme a seleção dos arquivos
        this.listFilesEl.addEventListener('selectionchange', e=>{
            ...
            switch(this.getSelection().length){
                ...
                break;
                //caso esteja selecionado somente 1 arquivo
                case 1:
                    this.btnDelete.style.display = 'block';
                    this.btnRename.style.display = 'block';
                break;
                //caso esteja selecionado vários arquivos
                default:
                    this.btnDelete.style.display = 'block';
                    this.btnRename.style.display = 'none';
        ...
        //evento do botão "Enviar Arquivos"
        ...
        this.inputFilesEl.addEventListener('change', event=>{
            ...
            //trava o botão enquanto o upload estiver acontecendo
            this.btnSendFileEl.disable = true;

            //chama o metodo de upload dos arquivos e trata a resposta das promessas com o then
            this.uploadTask(event.target.files).then(responses =>{
                ...
                responses.forEach(resp => {
                    ...
                    //adiciona um novo item da coleção que tem no firebase
    ...
    //Metodo que retorna os elementos que estão selecionados
    getSelection(){
    ...
    //Método de ações depois que o upload terminar
    uploadComplete(){

        //fecha o modal
        this.modalShow(false);

        //zera o campo do click
        this.inputFilesEl.value = '';

        //libera o botão qdo o upload terminar
        this.btnSendFileEl.disable = true;

    }

    //pega a referencia do Firebase de onde nós queremos salvar
    getFireBaseRef(path){
    ...
    //metodo para mostrar/esconder a barra de progresso
    modalShow(show = true){
    ...
    //Metodo que contem as funcionalidades do ajax
    ajax(url, method = 'GET', formData = new FormData(), onprogress = function(){}, onloadstart = function(){}){
        ...
        return new Promise((resolve, reject)=>{
            ...
            let ajax = new XMLHttpRequest();
                ...
                //Abre a conexão via POST
                ajax.open(method, url);

                ajax.onload = event =>{                    

                    try{
                        //console.log("resposta do ajax:", ajax.responseText);
                ...
                //A hora que começa a enviar para calcularmos os tempo estimado de término de envio para mostrar na barra de progresso
    ...
    uploadTask(files){
        ...
        //o files é uma coleção e precisamos transformá-lo em array
        [...files].forEach(file=>{
            ...
            // Se eu fizer um upload de 10 arquivos, farei um push de 10 promessas dentro do promises
            promises.push(promise);
        });

        //O promise.all recebe um array de "promessas"
        //Então ele trata uma por uma podendo "resolve-las" ou dar erro
        //Aqui que executa tudo efetivamente
        return Promise.all(promises);
    ...
    //formata o tempo para ser exibido o tempo restante de upload
    ...
    getFileView(file, key){
        ...
        //cria uma chave com o dataset e atribui a chave que foi passada por parametro (chave do firebase)
        li.dataset.key = key;

        //cria uma variavel com o dataset e atribui o file que foi passado por parametro (file do firebase)
        // o file é um objeto, mas dentro do dataset só dá pra guardar string
    ...
    readFiles(){
        ...
        this.getFireBaseRef().on('value', snapshot =>{
            ...
            snapshot.forEach(snapshotItem => {
                ...
                //console.log('chave: ',key,'dados: ', data);
    ...
    //Método que lê os arquivos contidos dentro de uma determinada pasta
    openFolder(){

        if(this.lastFolder) this.getFireBaseRef(this.lastFolder).off('value');

        //le os arquivos dentro da pasta
        this.readFiles();
    }
    ...
    initEventsLi(li){
        ...
        li.addEventListener('dblclick', e=>{          
            ...
            switch(file.type){
                ...
                case 'folder':
                    //adiciona mais uma posição do array que representa as pastas
                    this.currentFolder.push(file.name);
                    //lê os arquivos contidos dentro da pasta
                    this.openFolder();
                break;
        ...
        li.addEventListener('click', e=>{
            ...
            //se a tecla shift estiver pressionada para selecionar varios
            if(e.shiftKey){
                ...
                //como o
                //se tiver o primeiro selecionado 
                if(firstLi){

                    let indexStart;
                    let indexEnd;
                    let lis = li.parentElement.childNodes;//pega todos os li's (cada li é um arquivo)

                    //percorre os li's 
                    lis.forEach((el,index)=>{
                        //Pega o index do primeiro elemento clicado
                        if(firstLi === el) indexStart = index;
                        //Pega o index do ultimo elemento clicado
                    ...
                    //percorre os li's novamente com objetivo de selecionar todos os arquivos no "range" da seleção com o shift pressionado
                    lis.forEach((el,i)=>{

                       if(i>=index[0] && i<= index[1]){
                            //Para qdo clicarmos em algum arquivo, aparecer como "selecionado" (fica com fundo com outra cor)
            ...
            //se a tecla control estiver pressionada para selecionar alguns
            if(!e.ctrlKey){
